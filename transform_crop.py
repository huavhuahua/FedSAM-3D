...
import numpy as np
from torchio import SpatialTransform, Subject
from torchio.utils import parse_spatial_shape
from torchio.transforms import Crop, Pad

TypeTripletInt = Tuple[int, int, int]
TypeSixBounds = Tuple[int, int, int, int, int, int]

class CropOrPadWithRestore(SpatialTransform):
    """Modify the field of view by cropping or padding to match a target shape.
    
    This transform modifies the affine matrix associated with the volume so that
    ...
    def apply_transform(self, subject: Subject) -> Tuple[Subject, Optional[TypeSixBounds], Optional[TypeSixBounds]]:
        ...
        reference_affine = subject["image"].affine
        for image in subject.get_images(intensity_only=False):
            image.affine = reference_affine.copy()
            
        if padding_params is None:
            padding_params = (0, 0, 0, 0, 0, 0)
        if cropping_params is None:
            cropping_params = (0, 0, 0, 0, 0, 0)
        return subject, padding_params, cropping_params

    def restore(self, subject: Subject, padding_params: Optional[TypeSixBounds], cropping_params: Optional[TypeSixBounds]) -> Subject:
        if cropping_params is not None:
            pad = Pad(cropping_params, padding_mode=0)
            subject = pad(subject)  # type: ignore[assignment]
        if padding_params is not None:
            crop = Crop(padding_params)
            subject = crop(subject)  # type: ignore[assignment]
        return subject

# Example usage:
# transform = CropOrPadWithRestore((120, 80, 180), mask_name='heart_mask')
# transformed_subject, padding_params, cropping_params = transform(subject)
# restored_subject = transform.restore(transformed_subject, padding_params, cropping_params)
